...
program.command("branch").description("Set branch name").argument("[args...]", "Branch arguments (name or -M name)").action((args) => setBranch(...args));
...
program.command("push").description("Push changes to Rakibox Cloud").argument("[message]", "Commit message", "Cloud publish checkpoint update").action(async (message) => await pushRakibox(message));
